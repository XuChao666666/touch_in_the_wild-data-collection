...
class TactileSensorNodeLeft(Node):
    def __init__(self) -> None:     # -> None 没有返回值
        ...
        self.declare_parameter("tactile_sensor_name_left", "left_finger")  #  声明节点参数：在当前节点中声明一个名为"tactile_sensor_name_left"的参数，并设置默认值为"left_finger_real"
        ...
        self._prev = np.zeros((16, 16))     # 初始化类的成员变量self._prev（下划线私有变量，仅在类的内部使用），将其赋值为16 行，32 列的一个全为 0 的数组（矩阵）
    ...
    def _reader(self) -> None:  # 读取线程，且该方法没有返回值；
        ...
        while True:
            ...
            if len(line) < 10:  # 若当前一行长度小于10，则检查之前积累的block是否完整；
                if len(block) == 16 and all(len(r) == 16 for r in block):   # 若block符合条件（16行，每行16个元素），则存入samples
                    samples.append(np.array(block))
                    fps = 1 / (time.time() - t0) if t0 else 0.0     # 计算帧率（FPS）：当前时间与上一次存储的时间差的倒数;为何第一次到这里会将fps设置为0，因为第一次缺少时间戳，所以自动放弃第一帧的计算得到的帧率；
...
def main(args=None) -> None:
    ...
    executor = MultiThreadedExecutor(num_threads=1)     # 创建多线程执行器
